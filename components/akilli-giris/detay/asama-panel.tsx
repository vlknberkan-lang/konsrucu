 ...
 */
import { Check, CalendarPlus, MapPin, Flag } from 'lucide-react'
...
type Etkinlik = { id: string; tur: string; baslik: string; baslar: Date; yer: string | null; durum: string }
...
export function AsamaPanel({ sekme, dosyaId, asama, etkinlikler = [], prefill }: { sekme: Sekme; dosyaId: string; asama: Asama | null; etkinlikler?: Etkinlik[]; prefill?: { no?: string | null; birim?: string | null } }) {
  const m = META[sekme]
  ...
  return (
    <section className="mt-5 flex flex-col gap-4">
      ...
      {/* etkinlikler (toplantı/duruşma) — aşama kaydı varsa */}
      {asama && (
        <div className="overflow-hidden rounded-2xl border border-border bg-surface shadow-card">
          <div className="border-b border-border-subtle px-5 py-3"><span className="font-mono text-[9.5px] uppercase tracking-[0.14em] text-muted-foreground">{m.etkLabel} takvimi</span></div>
          <div className="flex flex-col gap-1.5 px-5 py-4">
            ...
            {etkinlikler.map((e) => (
              <div key={e.id} className="flex items-center gap-3 rounded-[11px] border border-border-subtle bg-surface-muted/40 px-3 py-2.5">
                <span className="font-mono text-[12.5px] font-bold tabular-nums text-kr-ink">{fmtDT(e.baslar)}</span>
          ...
          <form action={etkinlikKaydet} className="flex flex-wrap items-end gap-2 border-t border-border-subtle bg-surface-muted/30 px-5 py-3.5">
            ...
            <div><label className={LBL}>Tarih & saat</label><input type="datetime-local" name="baslar" className={`${INP} bg-surface`} /></div>
