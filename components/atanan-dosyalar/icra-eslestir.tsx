...
import { masterEslestir } from '@/app/(app)/akilli-giris/actions'

export function IcraEslestirButton() {
  const [open, setOpen] = useState(false)
  const [pending, setPending] = useState(false)
  const [sonuc, setSonuc] = useState<{ eslesen: number; borcluEklenen: number; bulunamayan: string[]; toplam: number } | null>(null)
  ...
  async function gonder() {
    ...
    try {
      const r = await masterEslestir(fd)
      if (r.ok) { setSonuc({ eslesen: r.eslesen, borcluEklenen: r.borcluEklenen, bulunamayan: r.bulunamayan, toplam: r.toplam }); router.refresh() }
  ...
  return (
    <>
      <button type="button" onClick={() => setOpen(true)} className="inline-flex items-center gap-2 rounded-[10px] border border-border bg-surface px-3.5 py-2.5 text-[13px] font-semibold text-foreground transition hover:border-kr/40 hover:text-kr">
        <FileSpreadsheet className="h-4 w-4" /> Excel'den Eşleştir
      </button>
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4" onClick={() => setOpen(false)}>
          <div className="w-full max-w-md overflow-hidden rounded-2xl border border-border bg-surface shadow-float" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-border-subtle px-5 py-3.5">
              <h3 className="font-display text-[16px] font-extrabold">Excel'den Eşleştir</h3>
            ...
            <div className="px-5 py-4">
              <p className="mb-3 text-[12.5px] text-muted-foreground"><b className="text-foreground">Hukuk no</b> ile eşleşen dosyalarda <b className="text-foreground">boş alanlar</b> Excel'den doldurulur (icra no/daire, rücu sebebi/oran/tutar, tarihler, avukatlar) ve <b className="text-foreground">borçlusu olmayana</b> RÜCU MUHATABI'ndan borçlu eklenir. Mevcut / AI verisi ezilmez.</p>
              ...
              {sonuc && (
                <div className="mt-3 rounded-[11px] border border-success/30 bg-success-soft/40 px-3 py-2.5 text-[12.5px]">
                  <div className="flex items-center gap-1.5 font-semibold text-success"><Check className="h-4 w-4" /> {sonuc.eslesen}/{sonuc.toplam} dosya güncellendi · {sonuc.borcluEklenen} borçlu eklendi</div>
