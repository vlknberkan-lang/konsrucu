...
flowchart LR
  ...
  D --> I
  E --> F
  F --> K["Kayıt mailleri<br/>@konslaw.com'dan"]
  G --> SATIS["🚀 SATIŞA AÇIK"]
  H --> SATIS
  I --> SATIS
  K --> SATIS
...
### 1a. Domain: konslaw.com satın al (15 dk)

1. **Cloudflare Registrar**'a git → cloudflare.com → hesap aç (ücretsiz).
   *Neden Cloudflare? Maliyetine satar (~$10/yıl), gizli ücret yok, DNS yönetimi en rahatı.*
   TR alternatifi istersen: isimtescil.net / natro (biraz daha pahalı, TL öder).
2. "Register Domain" → `konslaw.com` ara → satın al.
   - `konslaw.com.tr` de boşsa **onu da al** (yıllık ~₺300, marka koruması).
   - Öneri: `konslaw.net` ve `konslaw.co`'ya para verme, gerek yok.
3. Satın alınca **bana söyle** → Vercel'e bağlama talimatını vereceğim
   (Cloudflare DNS'e 2 kayıt eklemek: `A @ 76.76.21.21` ve `CNAME www cname.vercel-dns.com` —
   ben Vercel tarafını hazırlarım, sen kopyala-yapıştır yaparsın).

> ⚠️ **Marka kontrolü (5 dk):** Google'da "konslaw" ara + turkpatent.gov.tr'de
> marka sorgula. Çakışan bir hukuk yazılımı/bürosu var mı bak. Tescil başvurusunu
> (9. ve 42. sınıf) bu hafta bir marka vekiliyle başlat — acil değil ama geciktirme.
...
| **Kurumsal kutu** (info@konslaw.com'u okuyup yazmak) | **Zoho Mail** | ücretsiz (5 kutu) |

**Resend kurulumu:**
1. resend.com → Sign up (GitHub hesabınla 1 dk).
2. "Domains" → "Add Domain" → `konslaw.com` yaz.
3. Sana 3-4 DNS kaydı gösterecek (SPF, DKIM). Bunları Cloudflare DNS'e
   ...
   *Bu kayıtlar "bu maili gerçekten konslaw.com gönderiyor" imzasıdır — spam'e düşmemenin anahtarı.*
...
5. Ben `mail.ts`'e Resend'i bağlarım + **Supabase Auth SMTP ayarını** Resend'e çeviririm
   (böylece kayıt doğrulama maili de `no-reply@konslaw.com`'dan, KonsLaw şablonuyla gider —
   şu an Supabase'in kendi adresinden gidiyor, amatör görünüyor).

**Zoho Mail (opsiyonel, yarın da olur):** zoho.com/mail → Free plan → konslaw.com bağla →
...
- [ ] konslaw.com → siteyi açıyor (Vercel bağlandı)
- [ ] Kayıt ol → doğrulama maili `no-reply@konslaw.com`'dan geldi
